...
const HomeDashboard = (props) => {
    ...
    const [selectedLayoutName, setSelectedLayoutName] = useState("Default_Layout");
    ...
    const [isUserNewStatus, setIsUserNewStatus] = useStorageState(true, "isUserNew");
    ...
    useEffect(() => {
        if (isAuthenticated) {
            ...
            const saved_dashboard_data = db.collection("saved_dashboards").doc(userID);
                ...
                docRef
                    ...
                    .then((doc) => {
                        if (doc.exists) {
                            ...
                            if (!mappedDashboards.includes(newLayoutName) && newLayoutName !== undefined) {
            ...
            .then((doc) => {
                if (doc.exists) {
                    ...
                    if (data) {
                        ...
                        setSelectedLayoutName(Object.keys(currentLayout).flat()[0].split(" ").join("_"));
                        routerHistory.push(`/dashboard/${userID}/${selectedLayoutName}/${urlTicker.toUpperCase()}`);
